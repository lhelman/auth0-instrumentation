...
module.exports = function getLogger(pkg, env, serializers, agent) {
  ...
  if (env.LOG_TO_FLUENTD) {
    var sender = require('fluent-logger').createFluentSender(null, {
      path: env.LOG_TO_FLUENTD[0] === '/' ? env.LOG_TO_FLUENTD : undefined,
      host: env.LOG_TO_FLUENTD[0] !== '/' ? env.LOG_TO_FLUENTD : undefined,
      port: env.LOG_TO_FLUENTD_PORT || 24224,
      timeout: 3.0,
      reconnectInterval: 60000, // 1 minute
      eventMode: 'PackedForward'
    });
    var stream = sender.toStream(pkg.name);

    bunyan_streams.push({
      name: 'fluentd',
      stream: stream,
      level: env.LOG_TO_FLUENTD_LEVEL,
      type: env.LOG_TO_FLUENTD_LOG_TYPE
    });
